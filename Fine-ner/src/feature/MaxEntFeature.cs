...
namespace msra.nlp.tr
{
    class MaxEntFeature : Feature
    {
        ...
        internal List<string> GetFeature(string mention, string context)
        {
            ...
            {
                int index = DependencyParser.GetDriver(pair.first, pair.second);
                if (index != -1)
                {
            ...
            {
                int index = DependencyParser.GetAdjModifier(pair.first, pair.second);
                if (index != -1)
                {
            ...
            {
                int index = DependencyParser.GetAction(pair.first, pair.second);
                if (index != -1)
                {
